...
public partial class Slime : Node2D {
	...
	public override void _Process(double delta) {
		...
		var paramsStruct = ConvertStructToBytes (
			settings with { delta = (float) delta, time = (float) time }
		);
		...
		var trailMapImage = Image.CreateFromData(settings.width, settings.height, false, Image.Format.Rgba8, trailMapReadData);
